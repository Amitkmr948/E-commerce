...
import { add } from "../../../redux/Slices/CartSlice"; // Import the add action
import { toast } from "react-hot-toast";
import "./Style.css";

const HomeGarden = () => {
  const [loading, setLoading] = useState(true);
  const homeGardenProducts = productsData.filter(
    (product) => product && product.category === "Home & Garden" // Defensive check
  );
  ...
  const handleProductClick = (name) => {
    navigate(`/products/${name}`); // Navigate using product name
  };
  ...
  return (
    <div className="bg-slate-200 text-richblack-500 h-full m-auto pt-10">
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-4xl font-bold text-center mb-10">Home & Garden</h1>{" "}
        {/* Updated heading */}
        {loading ? (
          ...
          <div className="products-grid grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
            {homeGardenProducts.length > 0 ? (
              ...
              <p className="text-center text-white">
                No home & garden products available.
              </p>
            )}
          </div>
        )}
